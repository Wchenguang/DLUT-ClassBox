...
def get_classes(name,pswd):
    ...
    classes = []
    while (1):
        judge = CB_Global.zk_judge_pa.search(r.text, pos=begin)
        if (None == judge):
            break
        if (_is_class_module(judge.group(0))):
            aclass = CB_Global.zk_cpa18.search(r.text, pos=begin)
            begin = aclass.end()
            ...
            aclass = CB_Global.zk_cpa7.search(r.text, pos=begin)
            begin = aclass.end()
            msglist = CB_Global.zk_mpa.findall(aclass.group(0))
...
#判断一门课程是否在那一周
def zk_is_theweek(aclass,theweek):
    #部分课程无周信息
    if('' == aclass['课程周']):
        return 0
    #获取课程信息 课程周 上下限
